            ...
            steps {
                script {
                    ...
                    dir('python-microservice-CD') {
                        sh "sed -i 's|image: devopsuses/myfrontend-service:.*|image: devopsuses/myfrontend-service:${env.DOCKER_TAG}|g' application/K8/CANARY-DEPLOYMENT-WITH-ARGO-ISTIO/my-frontend.yaml"

                        // Configure Git user and commit the changes
                        sh 'git config user.name "devopsusesl"'
                        sh 'git config user.email "devopsuse@gmail.com"'
                        sh 'git add application/K8/CANARY-DEPLOYMENT-WITH-ARGO-ISTIO/my-frontend.yaml'
